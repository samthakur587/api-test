...
# make a pydantic model to accept the data
...
# make a route add-data that takes a POST request with the data to be added taking data as list of strings
@app.post("/add-data/")
async def add_data(data: InputDataList):
    ...
    for i in range(len(data.data)):
        print(num_to_string[i+1])
        final_output.append("#case " + str(data.data[i].case_no) +"\n"+"##" + num_to_string[i+1] + " case \n" + "###" + data.data[i].name + "\n #### Background:\n" + data.data[i].background)
    
    print(final_output)
    for txt in final_output:
        print(txt)
